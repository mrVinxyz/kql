...
class DatabaseQuerySequenceTest {
    private lateinit var database: Database
    private val dbName = "query_sequence_test.db"
    ...
    @Test
    fun `test querySequence returns all entities as sequence`() = runBlocking {
        val sql = "SELECT * FROM test_entity ORDER BY pkId"

        val result = database.querySequence(sql) { row ->
            TestEntity(
        ...
        val sequence = result.getOrNull()
        
        val entities = sequence?.toList()
    ...
    @Test
    fun `test querySequence with no matching records returns null`() = runBlocking {
    ...
    @Test
    fun `test querySequence with projection returns selected columns`() = runBlocking {
        ...
        assertTrue(result is DatabaseResult.Success)
        val entities = result.getOrNull()?.toList()
    ...
    @Test
    fun `test querySequence handles SQL syntax errors`() = runBlocking {
        val invalidSql = "SELECT * FROM non_existent_table"

        val result = database.querySequence<TestEntity>(invalidSql) { row ->
            TestEntity(pkId = row.get<Int>("pkId")!!)
        }
    ...
    @Test
    fun `test querySequence with column index access`() = runBlocking {
        val sql = "SELECT pkId, stringValue, intValue, booleanValue FROM test_entity WHERE pkId = ?"

        val result = database.querySequence(sql, 2) { row ->
            TestEntity(
                pkId = row.get<Int>(0)!!,
