...
class UpdateTest {
    ...
    fun `test update with null values`() {
        ...
        val expectedSql = "UPDATE wizards SET guild = COALESCE(?, guild), specialization = COALESCE(?, specialization) " +
    ...
    fun `test update with between condition`() {
        ...
        val expectedSql = "UPDATE wizards SET level = ? WHERE power_level BETWEEN ? AND ? AND experience_points BETWEEN ? AND ?"
        assertEquals(expectedSql, update.sql)
