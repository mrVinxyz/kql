    ...
    inline operator fun <reified T : Any> get(arg: Any): T =
        when (T::class) {
            ...
            Int::class -> when (arg) {
                ...
                else -> error("Invalid type: $arg; Arg must be Int or String")
            } as T
 ...
 * Each iteration returns a `Row` object, allowing easy access to the data in each row.
 *
 ...
 * ```
 *
 * @property resultSet The [ResultSet] to iterate through.
 */
 ...
 * It supports common types such as:
 * - [String]
 * - [Int]
 * - [Long]
 * - [Float]
 ...
 * - [Boolean]
 *
 * If a type is not specifically handled, it defaults to using [java.sql.ResultSet.getObject].
 *
 * ### Example:
 * ```
